 ...
 */
class CRM_Contactupdatequeue_Form_Configuration extends CRM_Core_Form {
  ...
  public function setDefaultValues() {
    $current_config = CRM_Contactupdatequeue_Configuration::getRawConfig();
  ...
  public function postProcess() {
    ...
    CRM_Contactupdatequeue_Configuration::setRawConfig($current_config);

    // update the custom groups
    CRM_Contactupdatequeue_Configuration::synchroniseCustomFields();

    CRM_Core_Session::setStatus(E::ts("Contact Update Queue configuration was updated successfully."), E::ts("Configuration Saved"), 'info');
  ...
  protected function getHandlerClasses() {
    if (self::$_class2label === NULL) {
      ...
      foreach ($paths as $path) {
        $folder = $path . 'CRM/Contactupdatequeue/Handler';

        if (is_dir($folder)) {
          $files = scandir($folder);
          foreach ($files as $file) {
            if (preg_match("#(?P<class>^[a-z]+)[.]php$#i", $file, $match)) {
              $class_name = "CRM_Contactupdatequeue_Handler_{$match['class']}";
      ...
      foreach ($class_candidates as $class_name) {
        try {
          ...
          if ($class->isSubclassOf('CRM_Contactupdatequeue_ActivityHandler') && !$class->isAbstract()) {
