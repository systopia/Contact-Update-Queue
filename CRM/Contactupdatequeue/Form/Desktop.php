 ...
 */
class CRM_Contactupdatequeue_Form_Desktop extends CRM_Core_Form {
  ...
  public function buildQuickForm() {
    ...
    if (empty($activity_types)) {
      CRM_Core_Session::setStatus(E::ts("Please configure Contact Update Queue first."), E::ts('Warning'), 'info');
      CRM_Utils_System::redirect(CRM_Utils_System::url("civicrm/admin/contactupdatequeue"));
      return;
    }
    ...
    $session = CRM_Contactupdatequeue_Session::getSession();
    ...
    // Check if DONE....
    if ($session->isSiblingQueue() && empty($this->activity_id)) {
      // still need to check whether this was a sibling queue
      $session->returnFromSiblingQueue();
      $this->activity_id = $session->getCurrentActivityID($selected_types);
    }
    ...
    CRM_Contactupdatequeue_CustomData::labelCustomFields($this->activity);
    ...
    try {
      ...
      $url = CRM_Utils_System::url("civicrm/contactupdatequeue/desktop?reset=1");
    ...
    if (!$session->isSiblingQueue()) {
      $sibling_activities = CRM_Contactupdatequeue_Session::getSiblingActivityIDs($this->activity_id);
        ...
        if (count($sibling_activities) > 2) {
          CRM_Core_Session::setStatus(E::ts("There are %1 other changes scheduled for this contact. Click <strong><a href='%2'>HERE</a></strong> if you want to process those in one batch.",
              array(1 => count($sibling_activities) - 1,
                  2 => $url = CRM_Utils_System::url("civicrm/contactupdatequeue/desktop", "reset=1&restart=1&sibling_queue={$this->activity_id}"))),
          ...
          CRM_Core_Session::setStatus(E::ts("There is another change scheduled for this contact. Click <strong><a href='%1'>HERE</a></strong> if you want to process both in one batch.",
              array(1 => $url = CRM_Utils_System::url("civicrm/contactupdatequeue/desktop", "reset=1&restart=1&sibling_queue={$this->activity_id}"))),
              E::ts("Related Updates Scheduled!"),
              'warning');
        }
      }
    ...
    // add resources
    $this->assign('contactupdatequeue_icon_copy', E::url('resources/copy.png'));
    ...
    CRM_Core_Resources::singleton()->addStyleFile('be.aivl.i3val', 'css/contactupdatequeue.css');
  ...
  public function postProcess() {
    ...
    // check, if activity is still LIVE (See ContactUpdateQueue-14)
  ...
  protected function applyChanges() {
    ...
    CRM_Contactupdatequeue_CustomData::resolveCustomFields($activity_update);
    CRM_Contactupdatequeue_Session::log("UPDATE ACTIVITY " . json_encode($activity_update));
