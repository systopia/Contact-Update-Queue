...
class CRM_Contactupdatequeue_Session {
  ...
  /** @var CRM_ContactupdateQueue_Session the single session object  */
  ...
  public static function getSession() {
    if (self::$_singleton === NULL) {
      self::$_singleton = new CRM_Contactupdatequeue_Session();
    }
   ...
   * Works if CONTACTUPDATEQUEUE_DEBUG_LOGGING is TRUE
   */
  public static function log($message) {
    if (CONTACTUPDATEQUEUE_DEBUG_LOGGING) {
      ...
      CRM_Core_Error::debug_log_message("Contactupdatequeue [{$cache_key}]: {$message}");
    }
  }
  ...
  protected function getNext($grab_more_if_needed = TRUE, $after_timestamp = NULL) {
    ...
    $live_status_list = implode(',', CRM_Contactupdatequeue_Configuration::getConfiguration()->getLiveActivityStatuses());
    $next_activity_id = CRM_Core_DAO::singleValueQuery("
        SELECT cache.activity_id AS activity_id
        FROM contactupdatequeue_session_cache cache
    ...
    if (!$next_activity_id && $grab_more_if_needed) {
      ...
      $session_size = CRM_Contactupdatequeue_Configuration::getConfiguration()->getSessionSize();
  ...
  protected function releaseActivityID($activity_id) {
    ...
    $activity_id = (int) $activity_id;
    CRM_Core_DAO::singleValueQuery("DELETE FROM contactupdatequeue_session_cache WHERE activity_id = {$activity_id}");
  }
  ...
  protected function _reset($destroy_old_session = true) {
    ...
    $cache_key = sha1('contactupdatequeue' . microtime(TRUE) . rand());
  ...
  public function jumpToSiblingQueue($activity_id) {
    ...
    LEFT JOIN contactupdatequeue_session_cache session ON session.activity_id = activity.id
    ...
    foreach ($sibling_activity_ids_free as $activity_id) {
      ...
      CRM_Core_DAO::executeQuery("DELETE FROM contactupdatequeue_session_cache WHERE activity_id = {$activity_id}");
      // then: add to sub queue
      CRM_Core_DAO::executeQuery("INSERT INTO contactupdatequeue_session_cache (session_key, activity_id, expires) VALUES (%1, %2, %3)", array(
  ...
  protected function grabMoreActivities($max_count = 0, $after_timestamp = NULL) {
    ...
    $sql = "SELECT activity.id AS activity_id
            FROM civicrm_activity activity
            LEFT JOIN contactupdatequeue_session_cache ON activity.id = contactupdatequeue_session_cache.activity_id
            {$extra_join}
            WHERE activity.activity_type_id IN ({$activity_type_ids})
              AND activity.status_id IN ({$activity_status_ids})
              AND contactupdatequeue_session_cache.session_key IS NULL
  ...
  protected function destroySession($cache_key) {
    ...
    CRM_Core_DAO::executeQuery("DELETE FROM contactupdatequeue_session_cache WHERE session_key = '{$cache_key}'");
  }
  ...
  protected function purgeCache() {
    self::log("purgeCache");
    CRM_Core_DAO::executeQuery("DELETE FROM contactupdatequeue_session_cache WHERE expires < NOW()");
  }
  ...
  protected function set($name, $value) {
    $this->user_session->set($name, $value, 'contactupdatequeue_');
  }
  ...
  protected function get($name) {
    return $this->user_session->get($name, 'contactupdatequeue_');
  }
  ...
  public function getContinuationURL() {
    ...
    if (empty($activity_types)) {
      return CRM_Utils_System::url("civicrm/contactupdatequeue/desktop", "restart=1&reset=1");
    } else {
      return CRM_Utils_System::url("civicrm/contactupdatequeue/desktop", "restart=1&reset=1&types={$activity_types}");
    }
  }
