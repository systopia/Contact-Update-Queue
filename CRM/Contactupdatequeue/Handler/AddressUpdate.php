 ...
 */
class CRM_Contactupdatequeue_Handler_AddressUpdate extends CRM_Contactupdatequeue_Handler_DetailUpdate {

  public static $group_name = 'contactupdatequeue_address_updates';
  ...
  public function applyChanges($activity, $values, $objects = array()) {
    ...
    $action = explode(' ', CRM_Utils_Array::value('contactupdatequeue_address_updates_action', $values, ''));
    ...
    switch ($action[0]) {
      case 'add':
        ...
        // update contactupdatequeue job data
    ...
    if (!empty($address_update)) {
      ...
      CRM_Contactupdatequeue_Session::log("ADDRESS UPDATE: " . json_encode($address_update));
...
//    $this->applyAddressSharingChanges($activity, $action, $values, $activity_update, $address_update);
  ...
  public function renderActivityData($activity, $form) {
    ...
    $form->assign('contactupdatequeue_address_fields', $field2label);
    $form->assign('contactupdatequeue_address_values', $form_values);
...
//    $this->adjustAddressSharingOptions($options, $activity);
    $form->add(
      'select',
      "contactupdatequeue_address_updates_action",
      E::ts("Action"),
      $options,
      TRUE,
    ...
    $form->setDefaults(array(
      "contactupdatequeue_address_updates_action" => $configuration->pickDefaultAction($options, $default_action)));

    $form->assign('contactupdatequeue_active_address_fields', $active_fields);

    // add JS code
    CRM_Core_Resources::singleton()->addVars('contactupdatequeue_address_update', [
        'addresses'       => $addresses,
    ...
    // add missing address sharing mitigation (see https://github.com/systopia/Contact-Update-Queue/issues/42)
    $form->assign('contactupdatequeue_address_sharing_mitigation', $this->getAddressSharingNote($address_submitted, $addresses, $activity));
  }
  ...
  public function getTemplate() {
    return 'CRM/Contactupdatequeue/Handler/AddressUpdate.tpl';
  }
  ...
  protected function renderAddressSharingPanel($activity, $form, $address_submitted) {
    ...
    if (!empty($activity["{$group_name}.shared_with_contact_id"])) {
      ...
      $form->add(
        'select',
        'contactupdatequeue_address_sharing_addresses',
        E::ts("Share with"),
      ...
      $form->add(
        'select',
        'contactupdatequeue_address_sharing_location_type',
        E::ts("Share"),
      ...
      // assign stuff
      $form->add('hidden', 'contactupdatequeue_address_sharing_contact_id', $other_contact['id']);
      $form->assign('contactupdatequeue_address_sharing_contact', $other_contact);
    }
  }
  ...
  protected function applyAddressSharingChanges($activity, $action, $values, &$activity_update, $address_update) {
    if (empty($values['contactupdatequeue_address_sharing_addresses'])) {
      // nothing to do here...
      return;
    }
    ...
    $group_name = $this->getCustomGroupName();
    $other_contact_id = $values['contactupdatequeue_address_sharing_contact_id'];

    // ok, let's see what the user wants:
    switch ($values['contactupdatequeue_address_sharing_addresses']) {
      ...
      case 'new': // CREATE NEW ADDRESS WITH OTHER CONTACT
        if (empty($address_id)) {
          ...
          CRM_Core_Session::setStatus(E::ts("Active address not found!", array(1 => $other_contact_id)), E::ts('Error'), 'error');
        } else {
          ...
          $address['location_type'] = $values['contactupdatequeue_address_sharing_location_type'];
          $this->resolveFields($address);
      ...
      default: // SHARE WITH EXISTING ADDRESS
        $shared_address_id = $values['contactupdatequeue_address_sharing_addresses'];
   ...
   * @see https://github.com/systopia/Contact-Update-Queue/issues/42
   */
