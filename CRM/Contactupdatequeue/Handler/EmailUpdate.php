 ...
 */
class CRM_Contactupdatequeue_Handler_EmailUpdate extends CRM_Contactupdatequeue_Handler_DetailUpdate {

  public static $group_name = 'contactupdatequeue_email_updates';
  ...
  public function getCustomGroupSpeficationFiles() {
    return array(__DIR__ . '/../../../resources/email_updates_custom_group.json');
  }
  ...
  public function applyChanges($activity, $values, $objects = array()) {
    ...
    $action = CRM_Utils_Array::value('contactupdatequeue_email_updates_action', $values, '');
    switch ($action) {
      case 'add_primary':
      ...
      case 'update':
        ...
        $email_update['id']         = $values['contactupdatequeue_email_updates_email_id'];
    ...
    if (!empty($email_update)) {
      ...
      CRM_Contactupdatequeue_Session::log("EMAIL UPDATE: " . json_encode($email_update));
  ...
  public function renderActivityData($activity, $form) {
    ...
    if ($existing_email) {
      $form->add('hidden', 'contactupdatequeue_email_updates_email_id', $existing_email['id']);
      $this->addCurrentValues($values, $existing_email);
    } else {
      $form->add('hidden', 'contactupdatequeue_email_updates_email_id', 0);
    }

    $this->applyUpdateData($form_values, $values, "{$prefix}%s");
    $form->assign('contactupdatequeue_email_values', $form_values);
    $form->assign('contactupdatequeue_email_fields', $field2label);
    ...
    $form->add(
      'select',
      "contactupdatequeue_email_updates_action",
      E::ts("Action"),
      $options,
      TRUE,
    ...
    $form->setDefaults(array(
      "contactupdatequeue_email_updates_action" => $configuration->pickDefaultAction($options, $default_action)));

    $form->assign('contactupdatequeue_active_email_fields', $active_fields);
  }
  ...
  public function getTemplate() {
    return 'CRM/Contactupdatequeue/Handler/EmailUpdate.tpl';
  }
