 ...
 */
class CRM_Contactupdatequeue_Handler_PhoneUpdate extends CRM_Contactupdatequeue_Handler_DetailUpdate {

  public static $group_name = 'contactupdatequeue_phone_updates';
  ...
  public function applyChanges($activity, $values, $objects = array()) {
    ...
    $action = CRM_Utils_Array::value('contactupdatequeue_phone_updates_action', $values, '');
    switch ($action) {
      case 'add_primary':
      ...
      case 'update':
        ...
        $phone_update['id']         = $values['contactupdatequeue_phone_updates_phone_id'];
    ...
    if (!empty($phone_update)) {
      ...
      CRM_Contactupdatequeue_Session::log("PHONE UPDATE: " . json_encode($phone_update));
  ...
  public function renderActivityData($activity, $form) {
    ...
    if ($existing_phone) {
      $form->add('hidden', 'contactupdatequeue_phone_updates_phone_id', $existing_phone['id']);
      $this->addCurrentValues($values, $existing_phone);
    } else {
      $form->add('hidden', 'contactupdatequeue_phone_updates_phone_id', 0);
    }

    $this->applyUpdateData($form_values, $values, "{$prefix}%s");
    $form->assign('contactupdatequeue_phone_values', $form_values);
    $form->assign('contactupdatequeue_phone_fields', $field2label);
    ...
    $form->add(
      'select',
      "contactupdatequeue_phone_updates_action",
      E::ts("Action"),
      $options,
      TRUE,
    ...
    $form->setDefaults(array(
      "contactupdatequeue_phone_updates_action" => $configuration->pickDefaultAction($options, $default_action)));

    $form->assign('contactupdatequeue_active_phone_fields', $active_fields);
  }
  ...
  public function getTemplate() {
    return 'CRM/Contactupdatequeue/Handler/PhoneUpdate.tpl';
  }
