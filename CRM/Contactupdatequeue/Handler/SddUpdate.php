 ...
 */
class CRM_Contactupdatequeue_Handler_SddUpdate extends CRM_Contactupdatequeue_ActivityHandler {

  public static $group_name = 'contactupdatequeue_sdd_updates';
  ...
  public function getCustomGroupSpeficationFiles() {
    return array(__DIR__ . '/../../../resources/sdd_updates_custom_group.json');
  }
  ...
  public function renderActivityData($activity, $form) {
    ...
    $form->add('hidden', 'contactupdatequeue_sdd_updates_mandate_id', $existing_mandate['id']);
    ...
    $form->assign('contactupdatequeue_sdd_values', $form_values);
    $form->assign('contactupdatequeue_sdd_fields', $field2label);
    $form->assign('contactupdatequeue_sdd_mandate', $existing_mandate);
    ...
    $form->add(
      'select',
      "contactupdatequeue_sdd_updates_action",
      E::ts("Action"),
      array(0 => E::ts("Don't apply"), 1 => E::ts("Update Mandate")),
      TRUE,
      array('class' => 'huge crm-select2')
    );
    $form->setDefaults(array("contactupdatequeue_sdd_updates_action" => 1));
    ...
    $form->assign('contactupdatequeue_sdd_errors', json_encode($error_fields));
    $form->assign('contactupdatequeue_active_sdd_fields', $active_fields);
  }
  ...
  public function getTemplate() {
    return 'CRM/Contactupdatequeue/Handler/SddUpdate.tpl';
  }
  ...
  public function applyChanges($activity, $values, $objects = array()) {
    ...
    $action = CRM_Utils_Array::value('contactupdatequeue_sdd_updates_action', $values, '');
    ...
    if ($action) {
      ...
      if (!$mandate_processed) {
        ...
        CRM_Contactupdatequeue_Session::log("CREATE NEW " . json_encode($new_mandate));
  ...
  protected function renderCancelMandate($activity, $form, $existing_mandate) {
    ...
    $form->assign('contactupdatequeue_sdd_fields',        $field2label);
    $form->assign('contactupdatequeue_sdd_mandate',       $existing_mandate);
    $form->assign('contactupdatequeue_sdd_hide_original', 1);
    $form->assign('contactupdatequeue_sdd_is_cancel',     1);
