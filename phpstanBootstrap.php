...
foreach ($bootstrapFiles as $bootstrapFile) {
  if (str_ends_with($bootstrapFile, 'vendor/autoload.php')) {
    ...
    if (file_exists($civiCrmCoreDir)) {
      ...
      // Make function pathload() available.
      if (file_exists("$civiCrmCoreDir/mixin/lib/pathload-0.php")) {
        require_once "$civiCrmCoreDir/mixin/lib/pathload-0.php";
      }
...
function addExtensionDirToClassLoader(ClassLoader $loader, string $extensionDir): void {
  ...
  $civixFile = glob("$extensionDir/*.civix.php")[0] ?? NULL;
  if (NULL !== $civixFile) {
    $extShortName = basename($extensionDir);
    $dotPos = strrpos($extShortName, '.');
    if (FALSE !== $dotPos) {
      $extShortName = substr($extShortName, $dotPos + 1);
    }
    $extUtilClassName = 'CRM_' . str_replace(['-', '_'], '', ucwords($extShortName, '-_')) . '_ExtensionUtil';
    $loader->addClassMap([$extUtilClassName => $civixFile]);
  }
